function إنشاء_الشفرة(ast, مستوى = 0, عداد = { قيمة: 0 }, داخل_برنامج = false) {
    const مولدات = {
        ...
        طول: (عقدة) =>`Object.keys(${إنشاء_الشفرة(عقدة.قيمة, مستوى, عداد)}).length`,
        ...
        اضف: (عقدة) => {
            ...
            return `${إنشاء_الشفرة(عقدة.المتغير, مستوى, عداد)}.push(${إنشاء_الشفرة(
                عقدة.قيمة,
                مستوى,
                عداد
            )});`;
        ...
        ادرج: (عقدة) => {
            ...
            return `${إنشاء_الشفرة(عقدة.المتغير, مستوى, عداد)}.splice(${إنشاء_الشفرة(
                عقدة.موقع,
                مستوى,
                عداد
        ...
        تعابيرات: (عقدة) => {
            const list = عقدة.قيم.map((v) => إنشاء_الشفرة(v, 0, عداد)).join(", ");
            return `[${list}]`;
        },
        كائن: (عقدة) => {
            const props = عقدة.خصائص
                .map(
                    ({ key, value }) => `"${key}": ${إنشاء_الشفرة(value, 0, عداد)}`
                )
                .join(", ");
            return `{${props}}`;
        },
        قائمة: (عقدة) => {
            const قيم = عقدة.قيم
                .map(
                    ({ value }) => `${إنشاء_الشفرة(value, 0, عداد)}`
                )
                .join(", ");
            return `[${قيم}]`;
        },
        ...
        نص: (عقدة) => {
            عداد.قيمة++;
            const قيم = عقدة.قيم.map(({ value }) => إنشاء_الشفرة(value, 0, عداد));
