...
export function محلل_الفهرس(الرموز, الدالة_الام) {
    ...
    while (!تحقق(الرموز, "اقواس", "}")) {
        ...
        if (!رمز_المفتاح) رمي_خطأ(`لا يوجد مفتاح في الفهرس`, الرموز, الدالة_الام);
        المفتاح = رمز_المفتاح.القيمة.slice(1, -1);

        if (!تطابق(الرموز, "نقطتان"))
            رمي_خطأ(`لا يوجد ":" بعد المفتاح "${المفتاح}"`, الرموز, الدالة_الام);

        const القيمة = محلل_التعبير(الرموز, الدالة_الام);
        if (!القيمة) رمي_خطأ(`لا يوجد قيمة للفهرس "${المفتاح}"`, الرموز, الدالة_الام);
