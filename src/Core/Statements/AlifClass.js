...
import { حفظ_دالة } from "../AlifMemory.js";

export function محلل_صنف(الرموز, الدالة_الام) {
    تطابق(الرموز, "كلمة", "صنف");

    const اسم = تطابق(الرموز, "معرف");
    if (!اسم) رمي_خطأ("لا يوجد اسم للصنف", الرموز, الدالة_الام);
    ...
    if (تحقق(الرموز, "اقواس", "(")) {
        ...
        if (!تطابق(الرموز, "اقواس", ")"))
            رمي_خطأ(`لم يتم اغلاق القوس ")" بعد معاملات الصنف`, الرموز, الدالة_الام);
    }

    if (!تطابق(الرموز, "نقطتان")) رمي_خطأ(`توقعت ":" بعد تعريف الصنف`, الرموز, الدالة_الام);

    const اوامر = إدارة_المسافة_البادئة(الرموز, الدالة_الام, "صنف");
