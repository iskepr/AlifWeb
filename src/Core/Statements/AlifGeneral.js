...
export function محلل_عام_للاقواس(الرموز, الكلمة) {
    ...
    if (!تطابق(الرموز, "اقواس", ")"))
        رمي_خطأ(`لم يتم اغلاق القوس ")" بعد "${الكلمة}"`, الرموز);
...
export function محلل_عام_للكلمات_بقيمة(الرموز, الكلمة) {
    if (!الرموز || !Array.isArray(الرموز)) {
        throw new Error(
    ...
    let قيمة = محلل_التعبير(الرموز);

    return { نوع: الكلمة, قيمة };
}

export function محلل_عام_للاقواس_يسبقها_نقطة(الرموز, متغير, الكلمة) {
    if (!الرموز || !Array.isArray(الرموز)) {
        throw new Error(
